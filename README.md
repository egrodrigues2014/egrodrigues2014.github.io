...
These are run by hand, because they need a network or a browser and neither belongs
in a build gate. Last measured against the minified build:

| | |
|---|---|
| `mise run check-links` | 450 links, 0 broken |
| Lighthouse, mobile | accessibility **98** · SEO 92 · performance ~60 |
| Visual matrix | 3 languages × 3 viewports × light/dark — 0 console errors, 0 4xx, no horizontal scroll |

Lighthouse runs locally against `public/`, not against `hugo server`, which injects
livereload and distorts the numbers:

```bash
npx --yes http-server public -p 5099 --silent
```

```bash
CHROME_PATH="C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe" npx --yes lighthouse@12 http://127.0.0.1:5099/ --chrome-flags="--headless=new --no-sandbox --disable-gpu"
```

`--no-sandbox` is required here: without it Edge's headless mode dies with
"Network service crashed or was terminated". That same failure is why the CV and the
OG card are generated by pdfkit and resvg rather than by printing a page.

Known and deliberately not fixed:

- **`heading-order`**, 8 elements. The theme goes `h1` → `h3` → `h5` across five
  section templates. Fixing it means owning all five, for the difference between 98
  and 100.
- **`crawlable-anchors`**, 25 elements, 1 SEO point. `cards/skill.html` emits an
  `<a>` with no `href` when a skill has no `url`, which is not a link at all.
- **Performance ~60.** The bundle is 2.84 MB after mermaid was moved out of it, and
  most of what remains is Font Awesome's JS build — which is load-bearing, since
  there is no FA CSS or webfont in the output. Cutting it means owning
  `assets/scripts/application.js`, the theme's entry point.

Corrected here because it was reported twice as a defect and is not one: there is
**no 4px horizontal overflow at 375px**. The viewport emulator reports three
different widths for the same viewport — `innerWidth` 379, `visualViewport` 378,
`documentElement.clientWidth` 375 — and the navbar at `width: 100%` correctly
measures 379.2. `window.scrollTo(999, y)` leaves `scrollX` at 0: the page does not
scroll sideways at any of the three widths.
